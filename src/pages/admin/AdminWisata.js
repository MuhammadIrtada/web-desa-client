...
const AdminWisata = () => {
  ...
  const handleDelete = async (index) => {
    ...
    try {
      const res = await url.delete(`/wisata/${index}`, {
        headers: {
    ...
    <>
      <section>
        <div className="flex justify-between items-center mb-10">
          ...
          <Link to={"tambah"} className="flex ga`p`-2 items-center ">
            <svg
            ...
            <h2 className="font-semibold text-2l text-primary">
              Tambah Wisata
            </h2>
          </Link>
        </div>
        <div className="grid grid-cols-3 gap-x-3 gap-y-0 mb-6">
          {wisata &&
            wisata.slice(firstPostIndex, lastPostIndex).map((value) => (
